...
const Cursor = ({ color, shake, top, left, visible, canMove, disableTransition, seedCount, isTopTurn }) => {
  const playerLabel = isTopTurn ? 'P2' : 'P1';
  
  const svgCursor = (
    // Inline SVG code for LOWER cursor with tattooed player label
  ...
  const transitionClass = disableTransition ? 'no-transition' : '';
